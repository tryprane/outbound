...
export type ZohoAccountPayload = {
  accountId: string
...
export type ZohoMessagePayload = {
  messageId: string
  threadId?: string
  subject?: string
  summary?: string
...
export function safeZohoDate(value?: string | number | null) {
...
export function getZohoAuthUrl() {
  ...
  return `${resolveZohoAccountsBaseUrl()}/oauth/v2/auth?${params.toString()}`
}
...
export async function exchangeZohoCode(code: string) {
  ...
  const encryptedAccessToken = encrypt(payload.access_token)
  const encryptedRefreshToken = encrypt(payload.refresh_token)
  const tempAccount = {
    id: 'temp',
    type: 'zoho',
    email: 'temp',
    ...
    zohoRefreshToken: encryptedRefreshToken,
    zohoTokenExpiry: payload.expires_in ? new Date(Date.now() + payload.expires_in * 1000) : null,
    zohoRegion: inferRegionFromBaseUrl(payload.api_domain ? `${payload.api_domain}/api` : resolveZohoMailApiBaseUrl(null)),
  } as MailAccount
  ...
  return { email: account.email, accountId: account.id }
}
...
export function mapZohoMessageRecord(
  ...
  return {
    ...
    sentAt: safeZohoDate(message.sentDateInGMT),
    receivedAt: safeZohoDate(message.receivedTime),
